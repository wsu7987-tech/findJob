...
插件匹配到聊天页后，会显示当前匹配状态。未点击插件“开始”时，已确认任务保持在执行队列；点击“开始”后才发送消息或简历。简历请求成功后显示“简历发送成功”；失败时显示平台返回的失败原因。

工作台可按需同步 BOSS 历史聊天，并展示当前阶段、等待对象、等待时长、沟通来源、跟进建议与拒绝原因。“分析进展”可生成回复、跟进或拒绝原因询问建议，用户可以编辑后再发送。
