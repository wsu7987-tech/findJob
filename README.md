...
桌面端“岗位记录”作为与“岗位采集”同级的独立页面，支持关键词、城市、公司规模、公司行业、
融资阶段、详情状态、筛选状态、投递建议、求职进度、重复状态和日期范围筛选，并支持白名单字段排序和分页。
岗位名称可直接打开详情，列表显示是否建议投递；存在沟通记录时可跳转自动代聊。建议投递策略与筛选条件同行显示。
没有相关聊天的岗位在列表和详情页均可直接推入“待确认”，待确认后再由用户决定是否执行打招呼；公司管理和岗位记录会标记有过沟通的公司，点击即可打开按该公司名筛选的自动代聊会话。
删除岗位记录会同步清除投递建议，同时保留聊天、求职链路和申请记录。岗位记录页和当前岗位采集列表都支持单岗“采集详情/重新采集详情”；
岗位记录单独刷新详情不会增加岗位采集次数。
...
不建议岗位保留原结论并允许用户明确覆盖。桌面端“待确认”页面将打招呼、代聊和发简历任务合并展示：
岗位名称打开评估详情并可跳转历史岗位详情，代聊和发简历类型可进入关联聊天信息。代聊批准前会检查最新
消息并二次确认；已发送简历的重复待确认任务会直接取消。执行任务已连接独立 Chrome 执行器，可在用户授权后
执行 BOSS 默认招呼。
...
桌面端“自动代聊”工作台支持查看会话、生成消息建议、编辑消息和发送简历。消息编辑框只要有内容即可确认发送；AI 草稿可作为建议，不是发送前提。

发送消息和发送简历都先创建待确认任务，确认后进入执行队列。执行队列会保留消息内容或已选择的简历，任务可退回待确认继续调整。

发送简历直接使用会话页面已选择的简历：`encrypt_resume_id` 用于投递，文件名用于展示。无需再次读取附件简历列表。

插件匹配到聊天页后，会显示当前匹配状态。未点击插件“开始”时，已确认任务保持在执行队列；点击“开始”后才发送消息或简历。简历请求成功后显示“等待平台回显”，平台聊天动作确认后更新为“已发送”；失败时显示平台返回的失败原因。

工作台可按需同步 BOSS 历史聊天，并展示当前阶段、等待对象、等待时长、沟通来源、简历投递状态、跟进建议与拒绝原因。聊天同步发现新增消息后，固定规则会据此更新卡片：HR 主动打招呼或回复后，只要尚未实际发送简历且候选人未拒绝，进入“待发送简历”；HR 的附件邀请卡片作为独立动作保存，并绑定原卡片消息 ID，发送时执行邀请预检和接受流程；我方附件发送、HR 接收和 HR 查看附件分别更新简历状态。没有新增消息时保留原卡片结果。“规则分析”只重算已同步的本地聊天消息，不读取 BOSS 页面、不调用 AI；“分析进展”继续用于生成回复、跟进或拒绝原因询问建议，用户可以编辑后再发送。

会话页可人工标记“已约面试”。“拒绝”按钮会区分“我拒绝”和“招聘方拒绝”，两者都会结束当前流程并保留拒绝方；简历投递状态独立于岗位主阶段，因此后续进入评估或面试时仍可看到简历是否已发送、接收或查看。

会话栏的设置按钮提供全局“消息转义规则”。历史同步、实时监听和会话列表更新共用该规则：岗位卡片和附件状态会展示为中立会话动作，竞争者 PK 推广会被过滤；中立动作不会被标记为 HR 发言或触发自动回复。规则修改仅作用于后续同步消息，可在弹窗中恢复内置默认配置。

自动代聊页的“按时间批量更新”复用求职数据更新的 Scope：选择开始时间并统计后，页面分别显示待同步聊天和待补采岗位。聊天批量会同步聊天并补齐其关联岗位，岗位批量只补采范围内额外缺失的岗位详情；两种任务均固定最多处理 20 条，完成后用户再次点击对应按钮才会开始下一批。任务进度每秒刷新一次；岗位采集开始前等待 1–3 秒，处理相邻记录时等待 2–5 秒。
...
桌面端“今日行动”根据正式岗位进展、聊天等待状态、跟进建议、活动事件和待审核草稿动态生成当天的处理队列。
行动包括回复面试、发送或补充简历、回复招聘方、审核草稿、跟进招聘方和询问拒绝原因；列表由后端按可解释的
紧急程度、到期时间和等待时间稳定排序，支持按行动类型和优先级筛选，以及稍后处理、忽略、标记已处理和恢复。

“开始处理”和单项主按钮进入已有自动代聊会话。页面也可勾选当前筛选结果中的回复、面试回复、跟进和询问原因
行动批量生成草稿。服务端会逐项重新核对当前业务触发，已失效项跳过，已有生成中或待审核任务复用；每项独立处理，
单项失败不会影响其他岗位。批量完成后页面重新读取当前行动，已生成项目进入“审核草稿”。

单条和批量生成共用回复任务的业务触发标识与数据库唯一约束，重复点击、请求重试和并发请求不会为同一触发创建
多个有效草稿。已确认发送的跟进或拒绝原因询问只消费当次触发，新招聘方消息、新跟进建议或重新打开后的新拒绝事件
仍可形成新行动。生成草稿只到待审核状态，只有用户在自动代聊中确认后才会创建发送动作；发送简历不参与批量草稿。

## 求职分析

桌面端“求职分析”按 Asia/Shanghai 自然日展示主动联系、获得回复、发送简历、简历查看、约面、
拒绝、岗位关闭和 Offer 等历史指标。同一岗位在同一指标中只计一次；主动求职漏斗以所选日期内
首次主动联系的岗位为基础，并只观察统计结束日期前发生的后续进展。招聘方主动联系单独统计，
不会进入主动联系回复率。

核心指标和拒绝原因均可打开岗位明细。明细与主数字共用同一组岗位选择逻辑，并显示岗位标题、
公司、当前进展和匹配时间；拒绝明细继续显示 HR 明确、AI 推测或原因未知及原因摘要。当前待处理
读取最新岗位状态，不受历史日期范围影响；“等我回复”“等招聘方回复”和“建议跟进”可携带最小
筛选条件进入自动代聊。

历史趋势、漏斗、拒绝原因和来源表现来自正式 Activity；当前待处理来自 Pipeline 与最新聊天处理状态。
当前版本不包含求职策略 AI、岗位方向分类、跟进有效率、复杂同比环比和数据导出。
