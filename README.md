...
工作台可按需同步 BOSS 历史聊天，并展示当前阶段、等待对象、等待时长、沟通来源、简历投递状态、跟进建议与拒绝原因。聊天同步发现新增消息后，固定规则会据此更新卡片：HR 明确索要简历进入“待发送简历”，我方附件发送、HR 接收和 HR 查看附件分别更新简历状态。没有新增消息时保留原卡片结果。“规则分析”只重算已同步的本地聊天消息，不读取 BOSS 页面、不调用 AI；“分析进展”继续用于生成回复、跟进或拒绝原因询问建议，用户可以编辑后再发送。
