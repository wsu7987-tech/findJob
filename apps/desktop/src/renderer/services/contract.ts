...
const errorCategoryMap: Record<string, string> = {
  ...
  OUTPUT_FAILED: "导出失败",
  DB_FAILED: "数据写入失败",
  CODEX_OUTPUT_SCHEMA_INVALID: "Codex 输出 Schema 不兼容",
  CODEX_NOT_AUTHENTICATED: "Codex 未登录",
  CODEX_RATE_LIMITED: "Codex 请求受限",
  CODEX_MODEL_UNAVAILABLE: "Codex 模型不可用",
  CODEX_REASONING_EFFORT_UNSUPPORTED: "Codex 推理强度不支持",
  CODEX_TIMEOUT: "Codex 执行超时",
  CODEX_EVENT_FAILED: "Codex 返回失败事件",
  CODEX_TURN_FAILED: "Codex 轮次执行失败",
  CODEX_EVENT_INVALID: "Codex 输出事件无效",
  CODEX_PROCESS_FAILED: "Codex 执行失败",
  PROFILE_AI_FAILED: "资料 AI 调用失败",
  PROFILE_AI_OUTPUT_INVALID: "资料 AI 输出不符合契约",
  PROFILE_ANALYSIS_FAILED: "资料分析执行失败",
  CANCELLED: "已取消",
  UNKNOWN: "未知错误"
};
