...
const clampPageNumber = (pageNumber: number, sectionCount: number) => {
  if (sectionCount <= 0) {
    return Math.max(1, pageNumber);
  }
  return Math.min(Math.max(1, pageNumber), sectionCount);
};

export const useWebDraftStore = defineStore("webDraft", () => {
  const draftsById = ref<Record<string, WebDraft>>({});
  ...
  const selectedParser = ref<WebDraftParserName>("playwright_dom");
  const jobs = ref<WebReparseJob[]>([]);
  const activeJobId = ref<string | null>(null);
  const selectedParseResultId = ref<string | null>(null);
  const currentPageNumber = ref(1);
  const previewPageCache = ref<Record<string, WebDraftPreviewPage>>({});
      ...
      .map((item) => {
        ...
        const progressLabel = currentJob
          ? currentJob.total_pages > 0
            ? `${currentJob.processed_pages} / ${currentJob.total_pages} 段`
            : `${currentJob.processed_pages} / ? 段`
          : status === "failed"
            ? latestResult?.warnings.find((warning) => warning.trim().length > 0) ??
              "抓取失败，请查看警告信息。"
        ...
        return {
          ...
          latestResult,
          status,
          parserName: currentJob?.parser_name ?? latestResult?.parser_name ?? "playwright_dom",
          progressLabel, /*
            ? currentJob.total_pages > 0
              ? `${currentJob.processed_pages} / ${currentJob.total_pages} 段`
              : `${currentJob.processed_pages} / ? 段`
  ...
  const buildExportPayload = () => {
    ...
    const baseName = (draft.value.title || draft.value.source_name || "web-draft").replace(/[\\/:*?"<>|]/g, "-");
    return {
  ...
  const createDraft = async (payload: WebDraftCreateRequest) => {
    creating.value = true;
    error.value = null;
    try {
      const response = await api.createWebDraft(payload);
  ...
  const reparseDraft = async () => {
    ...
    error.value = null;
    try {
      const response = await api.reparseWebDraft(
        draft.value.id,
        {
          parser_name: selectedParser.value,
          session_profile_id: draft.value.session_profile_id ?? null
        },
        controller.signal
      );
      applyDraft(response.draft);
  ...
  const loadPreviewPage = async (pageNumber: number) => {
    ...
    const resolvedPageNumber = clampPageNumber(pageNumber, activePreviewResult.value.section_count);
    ...
    try {
      const response = await api.getWebDraftPreviewPage(
        draft.value.id,
  ...
  const refreshJobs = async () => {
    const response = await api.listWebReparseJobs();
    ...
    for (const job of response.jobs) {
      if (!draftsById.value[job.draft_id]) {
        try {
          const draftResponse = await api.getWebDraft(job.draft_id);
    ...
    pollTimer = setTimeout(async () => {
      try {
        ...
        for (const draftId of activeDraftIds) {
          try {
            const refreshed = await api.getWebDraft(draftId);
            applyDraft(refreshed.draft, {
  ...
  const saveCurrentResult = async () => {
    ...
    error.value = null;
    try {
      const response = await api.saveWebDraftParseResult(draft.value.id, activePreviewResult.value.id);
      applyDraft(response.draft);
  ...
  const commitDraft = async (payload?: PoolCommitMetadataRequest) => {
    ...
    error.value = null;
    try {
      const response = await api.commitWebDraft(draft.value.id, payload);
      removeDraftState(draft.value.id);
      connectionUnavailable.value = false;
  ...
  const discardDraft = async () => {
    ...
    try {
      ...
      await api.deleteWebDraft(draftId);
      connectionUnavailable.value = false;
    } catch (errorValue) {
      connectionUnavailable.value = errorValue instanceof NetworkError;
      error.value = (errorValue as Error).message;
  ...
  const openTask = async (draftId: string, jobId?: string) => {
    ...
    if (!draftsById.value[draftId]) {
      const response = await api.getWebDraft(draftId);
  ...
  const selectPreviewResult = (parseResultId: string): WebDraftParseResult | null => {
