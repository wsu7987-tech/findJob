...
- 文档状态：阶段 0–5 已完成；已增加本机外部 Codex 开发接入
- 更新日期：2026-09-11
...
- 开发接入：设置页支持分别安装或刷新 FineJob MCP 与 `finejob`、`finejob-profile` 两项 Skill；外部本机 Codex 新会话可使用项目当前源码和流程说明。
...
13. 内置 Codex 工作台通过独立运行凭证访问 FineJob；本机外部 Codex 使用专用回环 MCP 入口访问同一组业务工具，并保持现有业务版本、队列和敏感操作规则。
...
### 4.1 内置 TUI 的 MCP 调用身份边界

内置 TUI 的 MCP 虽然以 FineJob 子进程形式启动，但它与正在运行的 FineJob 后端不是同一个进程。内置 TUI 通过独立的内部 API 调用身份与后端协作：
...
因此，内置 TUI 的内部 API 同时执行回环地址、运行凭证、路由白名单和版本兼容性校验。本机外部 Codex 的开发接入见 4.3 节。
...
FineJob 提供独立于内置 TUI 的本机开发接入，适用于同一台电脑上新开的 Codex 会话。该接入由桌面端设置页管理，MCP 与 Skill 分别安装和刷新：

| 内容 | 安装/刷新结果 | 生效方式 |
| --- | --- | --- |
| FineJob MCP | 写入 Codex 的 `finejob` 服务配置，并生成指向当前开发工作区的本机启动器 | 新开 Codex 会话后读取最新工具清单 |
| `finejob` Skill | 同步岗位采集、筛选、JD 与投递建议流程 | 匹配岗位任务时加载 |
| `finejob-profile` Skill | 同步简历清洗、事实提取、QA、策略与搜索词流程 | 匹配简历分析任务时加载 |

使用者在 FineJob 的“设置 → 本机外部 Codex 接入”中按需点击“安装/刷新 FineJob MCP”或“安装/刷新 FineJob Skills”，再新开 Codex 会话。之后可以直接提出“用 FineJob 找岗位”或“用 FineJob 分析简历”，Skill 会按任务选择流程，MCP 会把搜索、筛选、JD、评估和沟通结果保存到 FineJob。

开发期间，MCP 实现改动由新会话启动当前项目源码生效；工具清单改动后刷新 MCP 并新开会话。Skill 文本或流程改动后单独刷新 Skills 并新开会话。两个刷新入口互不覆盖。

本机外部 MCP 通过回环地址调用 FineJob 后端，使用 `FINE_JOB_MCP_LOCAL_EXTERNAL=1` 标识本机自用模式，并且不携带内置工作台的 `Authorization` 运行凭证。内置工作台继续使用短期运行凭证，两条入口复用同一业务工具、记录、版本与队列。
...
内置 TUI 的 MCP 启动后先向 `/api/internal/codex/v1/handshake` 进行握手，之后所有请求都使用本次运行的调用凭证。握手和请求至少包含：
