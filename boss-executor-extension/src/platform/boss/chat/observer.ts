...
const assistantClientMids = new Set<string>();
...
export const markAssistantClientMid = (clientMid: string): void => {
  assistantClientMids.add(clientMid);
  window.setTimeout(() => assistantClientMids.delete(clientMid), 60_000);
};
...
const normalizeMessage = async (message: DecodedChatMessage): Promise<ChatObservedMessage | null> => {
  ...
  const contact = await resolveBossContactContext(peerUid);
